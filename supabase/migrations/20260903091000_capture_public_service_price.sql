-- Ensure public bookings store the service price selected at booking time.
...
BEGIN
  ...
  INSERT INTO public.appointments (
    service_id, service_name, service_price, customer_name, customer_email, customer_phone,
    start_time, end_time, status, notes, user_id
  ) VALUES (
    p_service_id, p_service_name, COALESCE(v_service.price, 0), trim(p_customer_name),
    lower(trim(p_customer_email)), trim(p_customer_phone), p_start_time, p_end_time,
    'confirmed', NULLIF(trim(p_notes), ''), p_user_id
  )
