...
const TeamPage = () => {
  ...
  const handleInvite = async (event: React.FormEvent) => {
    ...
    if (!workspace?.isOwner) {
      toast({ title: 'Acesso restrito', description: 'Somente o proprietário pode adicionar um profissional.', variant: 'destructive' });
      return;
    }
    if (!normalizedEmail || !normalizedEmail.includes('@')) {
      toast({ title: 'E-mail inválido', description: 'Informe um e-mail válido para o profissional.', variant: 'destructive' });
      return;
    }
    if (member) {
      toast({ title: 'Limite atingido', description: 'Seu plano permite o proprietário + 1 profissional.', variant: 'destructive' });
      return;
    }
  ...
  const handleRevoke = async () => {
    ...
    setSaving(true);
    const { error } = await (supabase as any).rpc('revoke_team_member');
    setSaving(false);
    if (error) {
    ...
    await loadMember();
    await refetch();
    toast({ title: 'Profissional removido', description: 'O acesso ao espaço compartilhado foi encerrado.' });
  };
    ...
    <DashboardLayout>
      <div className="container mx-auto max-w-3xl px-4 py-6 sm:py-8">
        ...
        {workspaceLoading || loading ? (
          <div className="flex h-40 items-center justify-center text-muted-foreground">Carregando equipe...</div>
        ) : !workspace?.isOwner ? (
          <Card>
            <CardHeader><CardTitle>Conta compartilhada</CardTitle><CardDescription>Você está usando o KENDRAH como profissional convidado.</CardDescription></CardHeader>
            <CardContent><div className="flex items-center gap-3 rounded-lg bg-kendrah-purple/5 p-4"><CheckCircle2 className="h-5 w-5 text-kendrah-purple" /><p className="text-sm">Você tem acesso à agenda, serviços, clientes e demais recursos compartilhados pelo proprietário.</p></div></CardContent>
          </Card>
        ) : (
          <div className="space-y-6">
            <Card>
              <CardHeader>
                <div className="flex items-start justify-between gap-4"><div><CardTitle className="flex items-center gap-2"><Users className="h-5 w-5 text-kendrah-purple" />Profissionais</CardTitle><CardDescription className="mt-1">Seu plano inclui você e mais 1 profissional.</CardDescription></div><Badge variant="secondary">{member ? '2 de 2' : '1 de 2'} profissionais</Badge></div>
              </CardHeader>
              <CardContent>
