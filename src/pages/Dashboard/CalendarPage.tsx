...
const CalendarPage = () => {
  ...
  return <div className="flex min-h-screen bg-kendrah-gray/30"><Sidebar /><div className="flex-1 min-w-0 p-4 sm:p-6 lg:p-8 overflow-auto lg:pl-64 pt-14 lg:pt-8">
    ...
    <TabsContent value="list" className="mt-0"><div className="kendrah-card p-3 sm:p-6"><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="font-semibold text-gray-900">Clientes do mês</h3><p className="text-sm text-gray-500">{monthlyEvents.length} agendamento(s) no mês atual</p></div><Button variant="outline" className="w-full sm:w-auto" onClick={handleCSV}><Download className="h-4 w-4 mr-2" /> Exportar clientes</Button></div><div className="space-y-3 md:hidden">{loading && <div className="py-8 text-center text-gray-500">Carregando agendamentos...</div>}{!loading && events.length === 0 && <div className="py-8 text-center text-gray-500">Nenhum agendamento ainda.</div>}{events.map(event=><div key={event.id} className="rounded-xl border bg-white p-4 shadow-sm"><div className="font-semibold text-gray-900">{event.resource.customer_name}</div><div className="mb-3 text-sm text-gray-500">{event.resource.customer_email}</div><div className="grid grid-cols-2 gap-2 text-sm"><div><span className="text-gray-500">Serviço</span><div>{event.resource.service_name}</div></div><div><span className="text-gray-500">Status</span><div>{event.resource.status === 'confirmed' ? 'Confirmado' : event.resource.status === 'pending' ? 'Pendente' : event.resource.status === 'cancelled' ? 'Cancelado' : 'Concluído'}</div></div><div><span className="text-gray-500">Data</span><div>{formatDate(event.start)}</div></div><div><span className="text-gray-500">Horário</span><div>{formatTime(event.start)} - {formatTime(event.end)}</div></div></div><div className="mt-4 flex gap-2"><Button className="flex-1 bg-green-600 hover:bg-green-700 text-white" onClick={() => openWhatsApp(event.resource.customer_phone, event.resource.customer_name)}><MessageCircle className="h-4 w-4 mr-2" /> WhatsApp</Button><Button variant="outline">Detalhes</Button></div></div>)}</div><div className="hidden overflow-x-auto md:block"><table className="min-w-[820px] w-full divide-y divide-gray-200"><thead><tr>{['Cliente','Serviço','Data','Horário','Status','WhatsApp','Ações'].map(h=><th key={h} className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{h}</th>)}</tr></thead><tbody className="bg-white divide-y divide-gray-200">{events.map(event=><tr key={event.id}><td className="px-6 py-4 whitespace-nowrap"><div className="font-medium text-gray-900">{event.resource.customer_name}</div><div className="text-sm text-gray-500">{event.resource.customer_email}</div></td><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{event.resource.service_name}</td><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatDate(event.start)}</td><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatTime(event.start)} - {formatTime(event.end)}</td><td className="px-6 py-4 whitespace-nowrap text-sm">{event.resource.status === 'confirmed' ? 'Confirmado' : event.resource.status === 'pending' ? 'Pendente' : event.resource.status === 'cancelled' ? 'Cancelado' : 'Concluído'}</td><td className="px-6 py-4 whitespace-nowrap"><Button variant="outline" size="sm" className="border-green-600 text-green-700 hover:bg-green-50" onClick={() => openWhatsApp(event.resource.customer_phone, event.resource.customer_name)}><MessageCircle className="h-4 w-4 mr-1" /> WhatsApp</Button></td><td className="px-6 py-4 whitespace-nowrap text-right"><Button variant="ghost" size="sm" className="text-kendrah-purple">Detalhes</Button></td></tr>)}</tbody></table></div></div></TabsContent></Tabs></div>
  </div></div>;
};
