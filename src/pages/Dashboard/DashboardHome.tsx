...
import { ArrowRight, CalendarDays, CheckCircle2, Clock3, DollarSign, Link2, Scissors, Settings2, UsersRound, UserRound, XCircle } from 'lucide-react';
...
const DashboardHome = () => {
  ...
  const revenue = completedAppointments.reduce(
    (total, appointment) => total + Number(appointment.service_price ?? priceByService.get(appointment.service_id) ?? 0),
    0
  );
        ...
        <main className="mx-auto w-full max-w-[1500px] p-4 sm:p-6 lg:p-8">
          <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <Link to="/dashboard/calendar">
              <Button className="h-11 rounded-xl bg-kendrah-purple px-5 font-semibold text-white shadow-sm hover:bg-kendrah-purple/90"><CalendarDays className="mr-2 h-4 w-4" />Abrir agenda</Button>
            </Link>
          </div>
          ...
          <section className="mb-7 rounded-2xl border border-kendrah-purple/20 bg-kendrah-purple/[0.06] p-5 shadow-sm sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-4">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-kendrah-purple/15 text-kendrah-purple"><DollarSign className="h-6 w-6" /></div>
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Faturamento realizado</p>
                  <p className="mt-0.5 text-sm text-muted-foreground/80">{completedAppointments.length} atendimento{completedAppointments.length === 1 ? '' : 's'} concluído{completedAppointments.length === 1 ? '' : 's'} × valor de cada serviço.</p>
                </div>
              </div>
